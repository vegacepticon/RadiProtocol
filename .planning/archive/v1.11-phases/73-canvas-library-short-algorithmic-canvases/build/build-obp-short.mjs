...
// === Start node (NO procedure preamble per D-04 + Pattern S5; no contrast fan-out per D-06 spirit — short ОБП is single-trunk) ===
let y = -200;
const start = c.start({ x: X, y });
y += STEP;

// === Section 1: ПАРЕНХИМАТОЗНЫЕ ОРГАНЫ (verbatim from short-жен MD line 10) ===
const sec1 = section({
...
// === Section 2: ЖЕЛЧНЫЙ ПУЗЫРЬ (verbatim from short-жен MD line 11) ===
const sec2 = section({
...
// === Section 3: ЗАБРЮШИННЫЕ ЛИМФОУЗЛЫ (verbatim from short-жен MD line 12) ===
const sec3 = section({
