...
- **D-19:** [informational] Author's vault path `Z:\documents\vaults\TEST-BASE\Protocols\` already contains: `ГМ 1.0.0.canvas`, `ОБП full 1.0.0.canvas`, `ОГК.canvas` (the existing reference, not `ОГК 1.10.0.canvas`), `ОЗП 1.0.0.canvas`, `ОМТ full 1.0.0.canvas`, `ПКОП 1.0.0.canvas`. The three new short canvases coexist alongside. (Observation, not an actionable decision — no plan task implements this; documents pre-existing vault state for context.)
