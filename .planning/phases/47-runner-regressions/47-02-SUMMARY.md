...
key-decisions:
  ...
  - "Verbatim scrollTop, no clamping [REVISED 2026-04-19 after UAT]: the initial implementation preserved scrollTop verbatim (todo's first option). UAT feedback showed the user actually wanted the todo's second option — scroll to the insertion point after each click so freshly inserted content is visible. Revision commit 95e7d0b flips the restore target from `this.pendingTextareaScrollTop` to `textarea.scrollHeight` (scroll to bottom). The pending field is now a flag only — its captured value is unused; non-null presence still gates the restore so unrelated renders keep default scrollTop=0."
