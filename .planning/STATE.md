...
Tests: 459 passed / 1 skipped / 7 failed (was 461/1/3 before Plan 03). +2 new Phase 49 D-05/D-08 regression tests green. +4 new red tests — all Plan-04-dependent (RUN-02 body-walk on e2, RUN-04 nested body-walk on e2, W4 long-body body-walk on e2, session-05 round-trip body-walk on e2 — all fail because `unified-loop-{valid,nested,long-body}.canvas` still carry pre-Phase-49 «проверка» labels on e2 body edges; under D-05 the runtime now dispatches those as exits instead of walking the body). Plan 05 is the combined gate; Plan 49-04 strips the fixture labels (D-20).
Next: /gsd-execute-plan 49 04 (fixture audit — strip "проверка" body-edge labels from unified-loop-valid/nested/long-body.canvas per D-20; create unified-loop-stray-body-label.canvas per D-16; optionally rename unified-loop-duplicate-exit.canvas).
Last activity: 2026-04-19 — Plan 49-03 shipped. Runtime (ProtocolRunner) and view (RunnerView) now speak the Phase 49 convention in lock-step with the validator. Zero runtime `edge.label === 'выход'` comparisons in src/runner/ or src/views/. Body-button captions now show target node's preview text via the shared `nodeLabel()` util — eliminates the `(no label)` fallback. Deviations: [Rule 3] added `RPNode` type import to the test file (plan anticipated this); [Rule 2] expanded the plan's inline ProtocolGraph snippet to include the required `canvasFilePath`, `reverseAdjacency`, `startNodeId` fields (plan's snippet was illustrative, real interface requires all six fields).
