...
describe('InlineRunnerModal — step counter (TASK #90)', () => {
  function makeStepCounterGraph() {
    const node = (id: string, kind: string = 'question') => ({ id, kind, x: 0, y: 0, width: 100, height: 60, questionText: id });
    return {
      canvasFilePath: 'test.rp.json',
      nodes: new Map([
        ['start', { id: 'start', kind: 'start', x: 0, y: 0, width: 100, height: 60, questionText: '' }],
        ['q1', node('q1', 'question')],
        ['a1', node('a1', 'answer')],
        ['q2', node('q2', 'question')],
        ['a2', node('a2', 'answer')],
  ...
  function setupWithCounter() {
    const { modal } = setupModal();
    (modal as any).graph = makeStepCounterGraph();
    const stepCounterEl = makeEl('div');
    (modal as any).stepCounterEl = stepCounterEl;
    return { modal, stepCounterEl };
  }

  it('idle state shows current=0, total=4', () => {
    const { modal, stepCounterEl } = setupWithCounter();
    (modal as any).updateStepCounter({ status: 'idle' });
    expect(stepCounterEl._text).toBe('Шаг 0 / 4');
    expect(stepCounterEl._attrs['aria-label']).toBe('Вопрос 0 из 4');
  });

  it('at-node with undoStackSize=1 shows current=2, total=4', () => {
    const { modal, stepCounterEl } = setupWithCounter();
    (modal as any).updateStepCounter({
      status: 'at-node',
      currentNodeId: 'q2',
    ...
    expect(stepCounterEl._text).toBe('Шаг 2 / 4');
    expect(stepCounterEl._attrs['aria-label']).toBe('Вопрос 2 из 4');
  });

  it('at-node with undoStackSize=0 shows current=1, total=4', () => {
    const { modal, stepCounterEl } = setupWithCounter();
    (modal as any).updateStepCounter({
      status: 'at-node',
      currentNodeId: 'q1',
    ...
    expect(stepCounterEl._text).toBe('Шаг 1 / 4');
    expect(stepCounterEl._attrs['aria-label']).toBe('Вопрос 1 из 4');
  });

  it('complete state shows current=4, total=4', () => {
    const { modal, stepCounterEl } = setupWithCounter();
    (modal as any).updateStepCounter({
      status: 'complete',
      finalText: 'done',
    });
    expect(stepCounterEl._text).toBe('Шаг 4 / 4');
    expect(stepCounterEl._attrs['aria-label']).toBe('Вопрос 4 из 4');
  });

  it('error state shows current=0, total=4', () => {
    const { modal, stepCounterEl } = setupWithCounter();
    (modal as any).updateStepCounter({
      status: 'error',
  ...
  it('awaiting-snippet-pick with undoStackSize=2 shows current=3, total=4', () => {
    const { modal, stepCounterEl } = setupWithCounter();
    (modal as any).updateStepCounter({
    ...
    expect(stepCounterEl._text).toBe('Шаг 3 / 4');
    expect(stepCounterEl._attrs['aria-label']).toBe('Вопрос 3 из 4');
  });

  it('awaiting-loop-pick with undoStackSize=3 shows current=4, total=4', () => {
    const { modal, stepCounterEl } = setupWithCounter();
    (modal as any).updateStepCounter({
  ...
  it('awaiting-snippet-fill with undoStackSize=2 shows current=3, total=4', () => {
    const { modal, stepCounterEl } = setupWithCounter();
    (modal as any).updateStepCounter({
      status: 'awaiting-snippet-fill',
      snippetId: 's1',
  ...
  it('returns early when graph is null', () => {
    const { modal } = setupModal();
    (modal as any).graph = null;
    const stepCounterEl = makeEl('div');
    (modal as any).stepCounterEl = stepCounterEl;
    (modal as any).updateStepCounter({
      status: 'at-node',
    ...
    expect(stepCounterEl._text).toBe('');
    expect(stepCounterEl._attrs['aria-label'] ?? null).toBeNull();
  });

  it('returns early when stepCounterEl is null', () => {
    const { modal } = setupModal();
    (modal as any).graph = makeStepCounterGraph();
    (modal as any).stepCounterEl = null;
    expect(() => {
      (modal as any).updateStepCounter({
        status: 'at-node',
        currentNodeId: 'q1',
