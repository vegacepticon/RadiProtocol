...
describe('ProtocolRunner.getSerializableState() (SESSION-01)', () => {
  ...
  it('serialized state has all required PersistedSession fields', () => {
    ...
    // NTYPE-04: snippetId and snippetNodeId no longer exist in the serialized state
    expect('snippetId' in serialized).toBe(false);
    expect('snippetNodeId' in serialized).toBe(false);
  });
});
